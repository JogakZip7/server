...
module.exports = (db) => {
   ...
   router.post("/groups/:groupId/posts", async (req, res) => {
      ...
      const {
         title, content, imageUrl, isPublic, location, moment
      } = req.body;
      ...
      try{
         ...
         const [result] = await db.execute(`INSERT INTO POST (userId, groupId, title, content, imageUrl, isPublic, location, moment, likeCount, commentCount, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0, NOW()`,
