  ...
  router.post("/", auth, async (req, res) => {
    try {
      ...
      const [result] = await db.execute(
        "INSERT INTO `GROUP` (owner, name, imageUrl, introduction, postCount, memberCount) VALUES (?, ?, ?, ?, 0, 1)",
        [ownerId, name, imageUrl, introduction]
      );

      await db.execute("INSERT INTO PARTICIPATE (userId, groupId) VALUES (?, ?)", [ownerId, result.insertId]);
